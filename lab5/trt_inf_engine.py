...
def show_performance(rslts: dict):
    ...
    print(f" Total time is {end-warm_up_start:.2f}s")
    print(f" Warm up time is {end-warm_up_start-inf_time:.2f}s")
    print(f" Inference time is {inf_time:.2f}s")
    print(f" FPS is {eval_num/(end-warm_up_start):.2f}")
    print(f" FPS(exclude warmup) is {eval_num/(inf_time):.2f}")

    print(f"Accuracy is {correct/eval_num:.2%}")
...
def draw_plot(bs: list, fps: list, fps_wo_wu: list, acc):
    ...
    fig.suptitle('TensorRT Performance under Various Batch Size',
                 fontweight='bold')
    ax1.set_title(f"Accuracy is {acc:.2%}")

    ax1.plot(bs, fps, '-o', label="FPS w/ warm up")
    ax1.plot(bs, fps_wo_wu, '-^', label="FPS w/o warm up")
    ...
    ax2.plot(bs,[1/x for x in fps] , '-o', label="latency w/ warm up")
    ax2.plot(bs,[1/x for x in fps_wo_wu], '-^', label="latency w/o warm up")
    #ax2.plot(bs, fps, '-o', label="FPS w/ warm up")
    #ax2.plot(bs, fps_wo_wu, '-^', label="FPS w/o warm up")
...
def run(params):
    ...
    for batch_size in bs_list:
        print('-'*66)
        print(f"[result] batch size {batch_size}.")
...
def params_loader():
    ...
    parser.add_argument("--dataset_path", type=str, default="/workspace/DLSR_LAB_learning/food11re/evaluation",
                        help="Path to evaluation dataset")
    parser.add_argument("--model_path", type=str,
                        default="/workspace/DLSR_LAB_learning/trainedModel/super_resolution.onnx",
